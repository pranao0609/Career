...
# ============================================================
# TOOLS: Search Functions (Updated for Groq compatibility)
# ============================================================

def search_google_courses(query: str, max_results: int = 5) -> List[Dict]:
    """Search for courses using web search fallback"""
    try:
        # For Groq implementation, we'll use a simpler approach
        # You can integrate with SerpAPI, Serper, or other search APIs
        return _get_fallback_courses(query)
    except Exception as e:
        logger.error(f"Course search error: {e}, using fallback")
        return _get_fallback_courses(query)

def search_youtube_tutorials(query: str, max_results: int = 3) -> List[Dict]:
    """Search for tutorials using web search fallback"""
    try:
        # For Groq implementation, use simpler approach
        return _get_fallback_videos(query)
    except Exception as e:
        logger.error(f"Video search error: {e}, using fallback")
        return _get_fallback_videos(query)
...
def _get_fallback_courses(query: str) -> List[Dict]:
    ...
    fallback_courses = [
        ...
        {
            "name": f"{query} Fundamentals",
            "url": "https://www.edx.org",
            "description": f"Learn {query} from industry experts",
            "type": "course",
            "provider": "edX"
        }
    ]
    return fallback_courses[:3]
...
def _get_fallback_videos(query: str) -> List[Dict]:
    ...
    fallback_videos = [
        ...
        },
        {
            ...
            "url": "https://www.youtube.com",
            "description": f"Quick {query} crash course for beginners",
            "type": "video",
            "channel": "LearnFast",
            "duration": "1-2 hours"
        }
    ]
    return fallback_videos[:2]
...
def generate_project_ideas(topic: str, skill_level: str = "beginner") -> List[Dict]:
    """Generate project ideas using Groq LLM"""
    try:
...
def _get_fallback_projects(topic: str, skill_level: str) -> List[Dict]:
    ...
    return [
        ...
        {
            "name": f"Intermediate {topic} Application",
            "description": f"Create a more complex application using {topic}",
            "skills": [f"Advanced {topic}", "Architecture", "Testing"],
...
# ============================================================
# CORE: Roadmap Generator (Updated for Groq)
# ============================================================

class RoadmapGenerator:
    """Generates comprehensive, personalized learning roadmaps using Groq"""
    ...
    def _extract_topics(self, query: str) -> List[str]:
        """Extract main learning topics from user query using Groq"""
        try:
            prompt = ChatPromptTemplate.from_template(
...
Maximum 6 topics. Each topic should be 1-3 words."""
    ...
    def _generate_structure(self, topics: List[str], original_query: str) -> Dict:
        """Generate complete roadmap structure with all details using Groq"""
